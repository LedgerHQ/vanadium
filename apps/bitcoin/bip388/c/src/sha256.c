/* Minimal pure-C SHA-256 (FIPS 180-4). Used only for the base58check
 * checksum on the 78-byte xpub serialization. */
...
void bip388_sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    ...
    for (int i = 0; i < 8; ++i) buf[56 + i] = (uint8_t)(bit_len >> ((7 - i) * 8));
    sha256_block(state, buf);
