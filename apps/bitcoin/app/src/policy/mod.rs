...
//!
//! A *signing program* is a tiny script whose SHA-256 is committed in the
//! chaincode of a key's xpub used in a wallet policy. When a transaction is
//! signed with such a key, the device retrieves the program from the PSBT,
//! evaluates it against a [`PolicyContext`] built from the transaction, and uses
//! the resulting [`SigningDecision`] to decide whether to sign — and whether to
//! do so without user confirmation.
//!
