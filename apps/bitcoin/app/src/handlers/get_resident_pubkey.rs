...
mod tests {
    use super::*;
    use crate::resident_key::{set_resident_key, FIXED_RESIDENT_KEY, FIXED_RESIDENT_PUBKEY};

    fn get_xpub(index: u16, display: bool) -> Vec<u8> {
        set_resident_key(FIXED_RESIDENT_KEY).unwrap();
    ...
    fn test_xpub_structure() {
        ...
        assert_eq!(
            &xpub[13..43],
            &[0u8; 30],
            "first 30 chaincode bytes must be zero"
        );

        // the last two bytes must match the index
        assert_eq!(
            &xpub[43..45],
            &index.to_be_bytes(),
            "last 2 chaincode bytes must encode the index"
        );

        // The pubkey must be the expected one
        assert_eq!(&xpub[45..78], &FIXED_RESIDENT_PUBKEY, "pubkey mismatch");
    }
    ...
    fn test_different_indices_differ_only_in_chaincode() {
        ...
        assert_eq!(
            &xpub0[45..],
            &xpub1[45..],
            "public key must not depend on index"
        );
