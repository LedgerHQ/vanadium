...
mod tests {
    ...
    use common::psbt::signing_policy::{PsbtSigningPolicyGlobalRead, ENGINE_ID_RISCV};

    use super::*;

    fn policy(program: &[u8]) -> SigningPolicy {
        SigningPolicy::new(ENGINE_ID_RISCV, 0, program.to_vec())
    }
    ...
    fn assert_preserves_maps(raw: &[u8]) {
        let signing_policy = policy(b"VPOL-test-image");
    ...
    fn inserts_policy_and_preserves_psbt_v0_maps() {
        ...
        let updated = insert_signing_policies(&raw, &[policy(b"VPOL-test-image")]).unwrap();
        let parsed = bitcoin::psbt::Psbt::deserialize(&updated).unwrap();
        let policies = parsed.get_signing_policies().unwrap();
        assert_eq!(policies.len(), 1);
        assert_eq!(policies[0].program, b"VPOL-test-image");
    }
