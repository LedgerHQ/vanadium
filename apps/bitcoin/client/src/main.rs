...
use vnd_bitcoin_client::{insert_signing_policies, BitcoinClient, BitcoinClientError};
...
async fn handle_cli_command(
    ...
    match &cli.command {
        ...
        CliCommand::SigningPolicyHash { policy } => {
            println!("{}", hex::encode(read_signing_policy(policy)?.hash()));
        }
    }
    Ok(())
}
