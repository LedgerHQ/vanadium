...
/// Subtype for a policy program entry (engine_id + version + length-prefixed program).
pub const PSBT_SIGNING_POLICY_GLOBAL_SCRIPT: u8 = 0x00;

/// Engine identifier for the riscv32imac engine.
///
/// `0x00` and `0x01` are reserved: they were used by abandoned prototypes (a
/// scripting-language embedding and a bespoke expression language, respectively).
pub const ENGINE_ID_RISCV: u8 = 0x02;
...
/// An owned signing policy supplied during account registration.
...
impl SigningPolicy {
    pub fn new(engine_id: u8, engine_version: u8, program: Vec<u8>) -> Self {
        Self {
            engine_id,
            engine_version,
            program,
    ...
    pub fn value_and_hash(&self) -> (Vec<u8>, [u8; 32]) {
        build_signing_policy_value(self.engine_id, self.engine_version, &self.program)
    }
...
/// compact_size_len || program`). A key is bound to this policy by deriving it along
...
/// Serialize an `(engine_id, engine_version, program)` triple into the value bytes
...
pub fn build_signing_policy_value(
    ...
    let mut value = Vec::with_capacity(2 + 9 + program.len());
    value.push(engine_id);
    value.push(engine_version);
    enc::VarInt(program.len() as u64)
...
pub fn parse_signing_policy_path(path: &[u32]) -> Option<(u32, u32, [u32; 4])> {
    ...
    Some((path[1] & !HARDENED_BIT, path[2] & !HARDENED_BIT, chunks))
}
...
pub fn set_signing_policy(
    ...
    let (value, hash) = build_signing_policy_value(engine_id, engine_version, program);
...
mod tests {
    ...
    fn build_and_parse_roundtrip() {
        let program = b"VPOL\x00\x00\x00\x00";
        let (value, hash) = build_signing_policy_value(ENGINE_ID_RISCV, 0x00, program);
        let parsed = SigningPolicyEntry::parse(&value).unwrap();
        assert_eq!(parsed.hash, hash);
        assert_eq!(parsed.engine_id, ENGINE_ID_RISCV);
    ...
    fn owned_policy_cbor_roundtrip_uses_canonical_hash() {
        let policy = SigningPolicy::new(ENGINE_ID_RISCV, 0x00, b"VPOL\x00".to_vec());
        let encoded = minicbor::to_vec(&policy).unwrap();
        let decoded: SigningPolicy = minicbor::decode(&encoded).unwrap();
        let (value, hash) = build_signing_policy_value(
            decoded.engine_id,
    ...
    #[test]
    fn engine_id_riscv_is_two_and_earlier_ids_stay_burned() {
        // 0x00 (rhai prototype) and 0x01 (bespoke expression language) must not be
        // reused, or previously-computed policy hashes would collide with new
        // programs under a different interpretation.
        assert_eq!(ENGINE_ID_RISCV, 0x02);
    }
    ...
    fn parse_rejects_length_mismatch() {
        // engine_id=2, version=0, len=10, but only 2 program bytes follow
        let bad = [0x02, 0x00, 0x0a, b'A', b'B'];
        assert!(SigningPolicyEntry::parse(&bad).is_err());
    }

    #[test]
    fn parse_handles_empty_program() {
        let (value, _) = build_signing_policy_value(ENGINE_ID_RISCV, 0x00, &[]);
        let parsed = SigningPolicyEntry::parse(&value).unwrap();
        assert_eq!(parsed.program, b"");
    }
    ...
    fn parse_rejects_non_policy_paths() {
        ...
        assert!(parse_signing_policy_path(&[84 | H, 1 | H, H, 0, 0, 0, 0]).is_none());
        // coin_type not hardened
        assert!(parse_signing_policy_path(&[SIGNING_POLICY_PURPOSE | H, 1, H, 0, 0, 0, 0]).is_none());
        // account not hardened
        assert!(
            parse_signing_policy_path(&[SIGNING_POLICY_PURPOSE | H, 1 | H, 0, 0, 0, 0, 0]).is_none()
        );
        // a chunk hardened
