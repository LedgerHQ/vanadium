...
use crate::{io::interrupt, Instruction};
...
pub struct CommEcallHandler<'a, const N: usize> {
    comm: Rc<RefCell<&'a mut ledger_device_sdk::io_new::Comm<N>>>,
    manifest: &'a Manifest,
...
impl<'a, const N: usize> CommEcallHandler<'a, N> {
    ...
    fn handle_send_buffer<E: fmt::Debug>(
        ...
        if size == 0 {
            ...
            SendBufferMessage::new(size as u32, buffer_type, &[]).serialize_to_comm(&mut tx);
            interrupt(tx)?;
            return Ok(());
        }
        ...
        while size > 0 {
            if first_chunk {
                ...
                let mut comm = self.comm.borrow_mut();
                let mut tx = comm.begin_tx();
                SendBufferContinuedMessage::new(&buffer[0..copy_size]).serialize_to_comm(&mut tx);
                interrupt(tx)?;
                size -= copy_size;
                g_ptr += copy_size as u32;
            }
    ...
    fn handle_xrecv<E: fmt::Debug>(
        ...
        while remaining_length != Some(0) {
            let mut comm = self.comm.borrow_mut();
            let mut tx = comm.begin_tx();
            ReceiveBufferMessage::new().serialize_to_comm(&mut tx);

            let command = interrupt(tx)?;
    ...
    fn handle_get_event<E: fmt::Debug>(
        ...
        if let Some((event_code, event_data)) = get_last_event() {
            ...
            // wait_for_ticker(&mut comm); TODO: implement this with io_new
