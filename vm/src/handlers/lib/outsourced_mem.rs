...
impl<'c, const N: usize> OutsourcedMemory<'c, N> {
    ...
    fn commit_page_at(&mut self, index: usize) -> Result<(), common::vm::MemoryError> {
        ...
        while n_processed_elements < n as usize {
            let mut tx = comm.begin_tx();
            CommitPageProofContinuedMessage::new().serialize_to_comm(&mut tx);
    ...
    fn load_page(
        ...
        let mut comm = self.comm.borrow_mut();
        let mut tx = comm.begin_tx();
        GetPageMessage::new(self.section_kind, page_index).serialize_to_comm(&mut tx);
        ...
        while n_processed_elements < n as usize {
            let mut tx = comm.begin_tx();
            GetPageProofContinuedMessage::new().serialize_to_comm(&mut tx);
